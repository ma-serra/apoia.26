...
interface PangeaSearchRawResponse {
...
const DEFAULT_ORGAOS = ["STF", "STJ"]
const EXTENDED_ORGAOS = ["STF", "STJ", "TST", "TRT4", "TRT1", "TRT2", "TRT3", "TRT5"]
const DEFAULT_ESPECIES = ["RG", "RESPR", "ADI ADC ADO ADPF", "SV", "SUM", "OJ", "IRR"]
...
const searchPangea = async (params: {
  query: string
  page: number
  orgaos: string[]
  especies: string[]
...
export const getPangeaTool = (_pUser: Promise<UserType>) => tool({
  ...
  execute: async ({ query, page, orgaos, especies, includeOthers, stripHtml = true, maxItems, debug }) => {
    ...
    const finalOrgaos = (orgaos && orgaos.length ? orgaos : DEFAULT_ORGAOS)
    const mergedOrgaos = includeOthers ? Array.from(new Set([...finalOrgaos, ...EXTENDED_ORGAOS])) : finalOrgaos
    const finalEspecies = (especies && especies.length ? especies : DEFAULT_ESPECIES)
    try {
