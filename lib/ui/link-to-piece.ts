...
export function addLinkToPieces(html: string, numeroDoProcesso: string, textos: TextoType[]) {
    return html.replace(/evento\s+(\d+)([^;)]*)/gmi, (match, eventNumber, rest) => {
        ...
        while ((labelMatch = labelRegex.exec(rest)) !== null) {
            ...
            if (foundTexto) {
                ...
                if (splited.length === 2)
                    id = splited[1]
                const link = `<span class="widgetlinkdocumento" data-idpiece="${foundTexto.id}" data-iddocumento="${id}" data-target="_blank" data-is2g="" data-numprocesso="${numeroDoProcesso}" data-mimetype="" data-uf="">${foundTexto.label}</span>`
